...
int32_t main() {
	...
	for(int v: dfs_order) {
		for (pii p : paths[v]){
			dbg(v, p.f, p.s);
			if(p.f != v) {
				// find which child p.f is in
				// add p to that child's things
				int c = hld->up(p.f, hld->depth[p.f]-hld->depth[v]-1);
				things[c].push_back(p);
				swap(p.f,p.s);
				c = hld->up(p.s, hld->depth[p.s]-hld->depth[v]-1);
				things[c].push_back(p);
			}
		}
		for(int i : adj[v]) {
